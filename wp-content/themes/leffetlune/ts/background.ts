...
type Flash = {x:number, y:number, size:number, moveX: boolean, moveY:boolean, grow: boolean}

export class Background
{
    private flashSize = {min: 2, max: 10};
    private flashColor1 = "rgba(255,255,255,0.8)";
    private flashColor2 = "rgba(255,255,255,0.3)";
    private flashMax = 10;
    private flashMoveProbability = 0.001;
    private flashGrow = 0.03;
    private flashSpeed = 0.1;
    ...
    moveFlash(flash: Flash):boolean
    {
        ...
        if(flash.moveX)flash.x+=this.flashSpeed;
        else flash.x-=this.flashSpeed;
        if(flash.moveY)flash.y+=this.flashSpeed;
        else flash.y-=this.flashSpeed;
        if(
            flash.x < 0-flash.size
     ...
     * @returns {number}
     */
    getRandomNumber(min:number=0, max:number=100, negative:boolean= false, probability: number = this.flashMoveProbability):number
    {
        if(negative){
     ...
     */
    getRandomPos(radius: number): {x:number, y:number}|undefined
    {
        if(!this.canvas)return;
        return {
